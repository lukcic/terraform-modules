...
resource "aws_iam_role_policy_attachment" "backup-service-aws-s3-backup-role-policy" {
  role       = aws_iam_role.aws-backup-service-role.name
  policy_arn = "arn:aws:iam::aws:policy/AWSBackupServiceRolePolicyForS3Backup"
}

resource "aws_iam_role_policy_attachment" "restore-service-aws-s3-backup-role-policy" {
  role       = aws_iam_role.aws-backup-service-role.name
